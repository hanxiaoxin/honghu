...
import menuMixin from './side-menu-mixin';
...
export default {
  ...
  computed: {
    ...
    isFirstLevel() {
      ...
      while (parent && parent !== this.rootMenu) {
        if (['HuSideSubmenu', 'HuSideMenuItemGroup'].indexOf(parent.$options.componentName) > -1) {
          isFirstLevel = false;
          break;
        } else {
  ...
  methods: {
    ...
    handleClick() {
      ...
      this.dispatch('HuSideMenu', 'submenu-click', this);
    },
    handleMouseenter(event, showTimeout = this.showTimeout) {
      ...
      this.dispatch('HuSideSubmenu', 'mouse-enter-child');
    ...
    handleMouseleave(deepDispatch = false) {
      ...
      this.dispatch('HuSideSubmenu', 'mouse-leave-child');
      clearTimeout(this.timeout);
      this.timeout = setTimeout(() => {
      ...
      if (this.appendToBody && deepDispatch) {
        if (this.$parent.$options.name === 'HuSideSubmenu') {
    ...
    handleTitleMouseenter() {
      ...
      title && (title.style.backgroundColor = this.rootMenu.hoverBackground) && (title.style.color = this.hoverTextColor);
      title.classList.add('hu-side-submenu-hover');
    },
    handleTitleMouseleave() {
      if (this.mode === 'horizontal' && !this.rootMenu.backgroundColor) return;
      const title = this.$refs['submenu-title'];
      title && (title.style.backgroundColor = this.rootMenu.backgroundColor || '') && (title.style.color = this.textColor);
      title.classList.remove('hu-side-submenu-hover');
    },
    updatePlacement() {
  ...
  render(h) {
    ...
    const popupMenu = (
      <transition name={menuTransitionName}>
        <div
          ref="menu"
          v-show={opened}
          class={[`hu-side-menu--${mode}`, popperClass]}
          ...
          <ul
            role="menu"
            class={['hu-side-menu hu-side-menu--popup', `hu-side-menu--popup-${currentPlacement}`]}
    ...
    const inlineMenu = (
      <hu-collapse-transition>
        <ul
          role="menu"
          class="hu-side-menu hu-side-menu--inline"
          v-show={opened}
          style={{ backgroundColor: rootMenu.backgroundColor || '' }}>
          {$slots.default}
        </ul>
