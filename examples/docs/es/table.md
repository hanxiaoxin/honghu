...
:::demo Utilice `row-class-name` en `hu-table` para agregar clases personalizadas a una fila en específico. Y entonces, podrá darle diseño con estas clases.
```html
<template>
  <hu-table
    :data="tableData"
...
<style>
  .hu-table .warning-row {
    background: oldlace;
  }

  .hu-table .success-row {
    background: #f0f9eb;
  }
</style>
...
:::demo Al configurar el atributo `height` de `hu-table`, puede fijar la cabecera de la tabla sin agregar otro código.
```html
<template>
  <hu-table
    :data="tableData"
...
:::demo El atributo `fixed` es utilizado en `hu-table-column`, este acepta un `Boolean`. Si es `true`, la columna será fijada a la izquierda. También acepta dos tipos: `left` y `right`, ambos indican donde debe ser fijada la columna.
```html
<template>
  <hu-table
    :data="tableData"
    ...
    <hu-table-column
      ...
      <template slot-scope="scope">
        <hu-button @click="handleClick" type="text" size="small">Detalle</hu-button>
        <hu-button type="text" size="small">Editar</hu-button>
      </template>
    </hu-table-column>
  </hu-table>
</template>
...
:::demo Al configurar el atributo `max-height` de `hu-table`, tu puedes fijar la cabecera de la tabla. La barra de desplazamiento únicamente se mostrará si la altura sobrepasa el valor de la altura máxima.
```html
<template>
  <hu-table
    :data="tableData"
...
:::demo Solo necesitas colocar `hu-table-column` dentro de un `hu-table-column`, de esta forma lograrás agruparlas.
```html
<template>
  <hu-table
    :data="tableData"
    ...
    <hu-table-column label="Información de entrega">
      <hu-table-column
        prop="name"
        label="Nombre"
        width="120">
      </hu-table-column>
      <hu-table-column label="Información de dirección">
        <hu-table-column
          prop="state"
          label="Estado"
          width="120">
...
<template>
  ...
  <div style="margin-top: 20px">
    <hu-button @click="setCurrent(tableData[1])">Seleccionar segunda fila</hu-button>
    <hu-button @click="setCurrent()">Limpiar selección</hu-button>
  </div>
</template>
...
<template>
  ...
  <div style="margin-top: 20px">
    <hu-button @click="toggleSelection([tableData[1], tableData[2]])">Cambia el estado de selección de la segunda y tercera fila.</hu-button>
    <hu-button @click="toggleSelection()">Limpiar selección</hu-button>
  </div>
</template>
...
:::demo Establezca el atributo `filters` y `filter-method` en `hu-table-column` haciendo esta columna filtrable. `filters` es un arreglo, y `filter-method` es una función que decide que filas se muestra. Esta tiene tres parámetros: `value`, `row` y `column`.
```html
<template>
  <hu-button @click="resetDateFilter">清除日期过滤器</hu-button>
  <hu-button @click="clearFilter">清除所有过滤器</hu-button>
  <hu-table
    ref="filterTable"
    :data="tableData"
    style="width: 100%">
  ...
  <hu-table
    ...
    <hu-table-column
      ...
      <template slot-scope="scope">
        <hu-button
          size="mini"
          @click="handleEdit(scope.$index, scope.row)">Editar</hu-button>
        <hu-button
          size="mini"
          type="danger"
          @click="handleDelete(scope.$index, scope.row)">Eliminar</hu-button>
      </template>
    </hu-table-column>
  </hu-table>
</template>
  ...
  <hu-table
    ...
    <hu-table-column
      ...
      <template slot-scope="scope">
        <hu-button
          size="mini"
          @click="handleEdit(scope.$index, scope.row)">Edit</hu-button>
        <hu-button
          size="mini"
          type="danger"
          @click="handleDelete(scope.$index, scope.row)">Delete</hu-button>
      </template>
    </hu-table-column>
  </hu-table>
</template>
...
:::demo Puede activar la fila expandible estableciendo la propiedad `type` a `expand` y Scoped Slots. La plantilla para `hu-table-column` se representará como el contenido de la fila expandible, y puede acceder a algunos atributos cuando está usando `Scoped Slots` en plantillas de columna personalizadas.
```html
<template>
  <hu-table
    :data="tableData"
...
| resizable             | especifica si el ancho de la columna puede ser redimensionado, funciona cuando `border` de `hu-table` está en `true` | boolean                           | —                             | false       |
